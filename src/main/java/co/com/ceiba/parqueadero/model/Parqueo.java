...
public class Parqueo {
	...
	public VehiculoEntity agregarVehiculo(ParqueoEntity parqueo) {
	...
	private boolean verificarPlacaYFechaVehiculo(VehiculoEntity vehiculoEntity) {
		...
		return (vehiculoEntity.getPlaca().charAt(0) == 'A' && (!"MONDAY".equals(fechaActual.getDayOfWeek().toString())
				|| !"SUNDAY".equals(fechaActual.getDayOfWeek().toString())));
	}
	...
	public int calcularValorAPagar(ParqueoEntity parqueoEntity) {
		Optional<VehiculoEntity> vehiculoEntity = vehiculoRepository
				.findById(parqueoEntity.getVehiculoEntity().getId());
		if (vehiculoEntity.isPresent()) {
			LocalDateTime fechaActual = LocalDateTime.now(clock);
			double horasTotales = (ChronoUnit.SECONDS.between(parqueoEntity.getFechaHoraDeEntrada(), fechaActual)
					/ 3600.0);
			ParqueoVehiculoFactory costoParqueoFactory = new ParqueoVehiculoFactory();
